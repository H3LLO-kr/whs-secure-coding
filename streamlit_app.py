...
def main():
    ...
    if st.session_state.logged_in:
        if st.session_state.user["role"] == 'admin':
            ...
            menu = ['Home', 'Add Product', 'Purchased products management']
            ...
            elif choice == "Purchased products management":
            ...
            if choice == 'Home':
                ...
                user_name = f'{st.session_state.user["username"]}'
                if st.button('Buy'):
                    buy_product_response = requests.get('http://localhost:8000/buy_product', params={"username": user_name, "address": user_address, "payment_info": user_payment_info, "product_name": selected_product})
                    if buy_product_response.status_code == 200:
                        st.success(buy_product_response.json()["message"])
                    else :
                        st.error(f'You failed to buy the product.')
                ...
                for purchased in purchaseds :
                    if st.session_state.user["username"] == purchased['username']:
                        st.write(f"Name: {purchased['product_name']}, Address: {purchased['address']}, Payment Info: {purchased['payment_info']}, Status: {purchased['status']}")
                        purchased_product_count += 1
                if purchased_product_count == 0:
                    st.write(f'There\'s nothing you purchased.')
                
                st.subheader('Change User Info')
                with st.form(key='edit_user_info_form'):
                    new_username = st.text_input('New Username', value=st.session_state.user["username"], disabled=True)
