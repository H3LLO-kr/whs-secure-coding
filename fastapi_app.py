...
def buy_product(conn, username, address, payment_info, product_name):
    cursor = conn.cursor()
    cursor.execute('INSERT INTO purchased (username, address, payment_info, product_name, status) VALUES(?, ?, ?, ?, ?)', (username, address, payment_info, product_name, "Not Sended"))
    conn.commit()
    return {"message": f"You have successfully purchased {product_name}! Address: {address}, Payment Info: {payment_info}"}

def get_purchased_products(conn):
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM purchased')
    purchaseds = cursor.fetchall()
    return [{"id": purchased[0], "username": purchased[1], "address": purchased[2], "payment_info": purchased[3], "product_name": purchased[4], "status": purchased[5]} for purchased in purchaseds]
...
@app.get("/buy_product")
async def buy_product_endpoint(username: str, address: str, payment_info: str, product_name: str):
    conn = create_connection()
    result = buy_product(conn, username, address, payment_info, product_name)
    conn.close()
    return result

@app.get("/purchased", response_model=List[dict])
async def get_purchased_products_endpoint():
    conn = create_connection()
    purchased = get_purchased_products(conn)
    conn.close()
    return purchased
